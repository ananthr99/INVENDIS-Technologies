...
function HeroTab({ data, patch }) {
  ...
  async function handleHeroImageUpload(file) {
    ...
    const filename = `sectors-hero.${ext}`
    ...
    try {
      ...
      await writeFileRawDirect(`images/hero/${filename}`, base64, `CMS: upload sectors hero image`, ghSha, token)
      let mainSha = null
      try { mainSha = (await readFile(`public/images/hero/${filename}`, token)).sha } catch {}
      await writeFileRaw(`public/images/hero/${filename}`, base64, `CMS: upload sectors hero image [skip ci]`, mainSha, token)
