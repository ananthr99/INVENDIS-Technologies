...
function HeroTab({ data, patch }) {
  ...
  async function handleHeroImageUpload(file) {
    ...
    const filename = `sectors-hero-${Date.now()}.${ext}`
    ...
    try {
      ...
      await writeFileRawDirect(`images/hero/${filename}`, base64, `CMS: upload sectors hero image`, null, token)
      await writeFileRaw(`public/images/hero/${filename}`, base64, `CMS: upload sectors hero image [skip ci]`, null, token)
