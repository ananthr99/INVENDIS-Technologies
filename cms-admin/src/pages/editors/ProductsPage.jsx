...
function HeroTab({ data, patch }) {
  ...
  async function handleHeroImageUpload(file) {
    ...
    const filename = `products-hero-${Date.now()}.${ext}`
    ...
    try {
      ...
      await writeFileRawDirect(`images/hero/${filename}`, base64, `CMS: upload products hero image`, null, token)
      await writeFileRaw(`public/images/hero/${filename}`, base64, `CMS: upload products hero image [skip ci]`, null, token)
